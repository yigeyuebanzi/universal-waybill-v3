    ...
    <AppShell>
      ...
      <Card className="mt-4">
        <table className="w-full text-left text-sm">
          ...
          <tbody>{rules.map((rule) => <tr key={rule.id} className="border-t"><td className="p-2">{rule.name}</td><td className="p-2">{rule.subType}</td><td className="p-2">{rule.conditionType}</td><td className="p-2">{rule.severity}</td><td className="p-2"><Badge tone={rule.enabled ? 'ok' : 'warn'}>{rule.enabled ? '启用' : '停用'}</Badge></td></tr>)}</tbody>
        </table>
      </Card>
    </AppShell>
  );
